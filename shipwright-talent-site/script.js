...
function typeLines(lines, index) {
  ...
  // Type as plain text growth is tricky with embedded tags, so reveal
  // the pre-built HTML in one pass per line with a short stagger instead
  // of a per-character type effect across markup boundaries.
  window.setTimeout(() => {
    line.innerHTML = full;
